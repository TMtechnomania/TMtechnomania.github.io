	...
	try {
		...
		try {
			...
			if (firstImage && firstImage.id && firstImage.asset)
				primaries.push({
					entry: firstImage,
					url: resolveAssetUrl(firstImage.asset),
					key: `${firstImage.id}::media`,
				});
			if (firstVideo && firstVideo.id && firstVideo.asset)
				primaries.push({
					entry: firstVideo,
					url: resolveAssetUrl(firstVideo.asset),
					key: `${firstVideo.id}::media`,
				});
			...
			if (primaries.length) {
				...
				await runWithConcurrency(
					...
					async (p) => {
						try {
							const existing = await getMediaEntry(p.key).catch(
								() => null,
							);
							...
							return await downloadAndStoreResource({
								url: p.url,
								assetPath: p.entry.id,
								kind: "media",
								manifestEntry: p.entry,
								keyOverride: p.key,
								maxAttempts: 3,
							});
						} catch (e) {
