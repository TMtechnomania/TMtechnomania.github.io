...
export function initToolsModal() {
	...
	header.innerHTML = `
        <div class="btn btn-icon-text btn-display modal-title">
            <span></span>
            <span>Tools Dashboard</span>
        </div>
    `;
	renderInlineIcon(
		header.querySelector(".modal-title span:first-child"),
		"assets/svgs-fontawesome/solid/keyboard.svg",
	);

	// Header Actions
	const actionsGroup = document.createElement("div");
	actionsGroup.className = "modal-header-actions";
	...
	closeBtn.onclick = () => closeToolsModal();
...
function createStatsWidget() {
	...
	widget.innerHTML = `
        <div class="widget-header">
            <div class="btn btn-icon-text btn-small btn-display">
                <span></span>
    ...
    `;
	renderInlineIcon(
		widget.querySelector(".widget-header .btn > span:first-child"),
		"assets/svgs-fontawesome/solid/chart-pie.svg",
	);
	renderInlineIcon(
		widget.querySelector("#cpu-card .stat-card-icon"),
		"assets/svgs-fontawesome/solid/microchip.svg",
	);
	renderInlineIcon(
		widget.querySelector("#mem-card .stat-card-icon"),
		"assets/svgs-fontawesome/solid/memory.svg",
	);
	renderInlineIcon(
		widget.querySelector("#storage-card .stat-card-icon"),
	...
	const updateStats = () => {
		...
		if (chrome.system?.cpu) {
			chrome.system.cpu.getInfo((info) => {
				const modelShort = info.modelName
					.replace(
						/\(R\)|\(TM\)|Core|Intel|AMD|Processor|CPU|@.*$/g,
						"",
					)
					.trim();
				widget.querySelector(".cpu-model").textContent =
					modelShort || "Unknown";
				widget.querySelector(".cpu-cores").textContent =
					info.numOfProcessors;
				widget.querySelector(".cpu-arch").textContent =
				...
				if (prevCpuInfo) {
					...
					widget.querySelector(
						"#cpu-card .stat-card-value",
					).textContent = `${avg}%`;
					widget.querySelector(
						"#cpu-card .stat-chart-bar",
					).style.width = `${avg}%`;
				}
				prevCpuInfo = info;
			});
		...
		if (chrome.system?.memory) {
			chrome.system.memory.getInfo((info) => {
				...
				widget.querySelector(
					"#mem-card .stat-card-value",
				).textContent = `${percent}%`;
				widget.querySelector(
					"#mem-card .stat-chart-bar",
				).style.width = `${percent}%`;
				widget.querySelector(".mem-used").textContent = `${used.toFixed(
					1,
				)} GB`;
				widget.querySelector(
					".mem-avail",
				).textContent = `${avail.toFixed(1)} GB`;
				widget.querySelector(
					".mem-total",
				).textContent = `${total.toFixed(1)} GB`;
			});
		}
		...
		if (chrome.system?.storage) {
			chrome.system.storage.getInfo((units) => {
				...
				const validUnits = units.filter((u) => u.capacity > 0);

				if (validUnits.length === 0) {
					widget.querySelector(
						"#storage-card .stat-card-value",
					).textContent = "0 Drives";
					return;
				}

				const promises = validUnits.map(
					(unit) =>
						new Promise((resolve) => {
							if (
								typeof chrome.system.storage
									.getAvailableCapacity === "function"
							) {
								chrome.system.storage.getAvailableCapacity(
									unit.id,
									(info) => {
										// Check for runtime error (e.g. permission issues or drive disconnected)
										if (chrome.runtime.lastError) {
											console.warn(
												"getAvailableCapacity failed:",
												chrome.runtime.lastError
													.message,
											);
											resolve({ unit, available: null });
										} else {
											resolve({
												unit,
												available:
				...
				Promise.all(promises).then((results) => {
					results.forEach(({ unit, available }) => {
						const name =
							unit.name.replace(/[^a-zA-Z0-9 :]/g, "") || "Drive";
						// Use binary GiB (1024^3) to match OS reporting
						const totalGiB = (unit.capacity / 1024 ** 3).toFixed(1);

						let usageText = `${totalGiB} GB`;

						if (typeof available === "number") {
							const used = unit.capacity - available;
							const usedGiB = (used / 1024 ** 3).toFixed(1);
							usageText = `${usedGiB} / ${totalGiB} GB`;
						}
					...
					widget.querySelector(
						"#storage-card .stat-card-value",
					).textContent = `${validUnits.length} Drive${
						validUnits.length > 1 ? "s" : ""
					}`;
				});
			});
		}
...
function createWorkspacesWidget() {
	...
	widget.innerHTML = `
        <div class="widget-header">
            ...
            <button class="btn btn-icon btn-small widget-action" id="ws-add-btn" title="New Workspace"><span></span></button>
        </div>
        <div class="widget-body">
            ...
            <div class="ws-create-view" style="display: none;">
                <input type="text" class="input ws-name-input" placeholder="Workspace name...">
                <div class="ws-tabs-select"></div>
                <div class="ws-create-actions">
                    <button class="btn btn-small btn-text ws-cancel-btn">Cancel</button>
                    <button class="btn btn-small btn-text primary ws-save-btn">Save</button>
                </div>
            </div>
        </div>
    `;
	renderInlineIcon(
		widget.querySelector(".widget-header .btn > span:first-child"),
		"assets/svgs-fontawesome/solid/layer-group.svg",
	);
	renderInlineIcon(
		widget.querySelector("#ws-add-btn span"),
	...
	const loadWorkspaces = () => {
		chrome.storage.local.get("workspaces", (res) => {
			...
			if (workspaces.length === 0) {
				list.innerHTML =
					'<div class="empty-msg">No workspaces yet</div>';
			} else {
				list.innerHTML = "";
				workspaces.forEach((ws, idx) => {
					...
					item.innerHTML = `
                        ...
                        <div class="ws-actions">
                            <button class="btn btn-icon btn-small ws-open" title="Open"><span></span></button>
                            <button class="btn btn-icon btn-small ws-delete" title="Delete"><span></span></button>
                        </div>
                    `;
					renderInlineIcon(
						item.querySelector(".ws-open span"),
						"assets/svgs-fontawesome/solid/arrow-up-right-from-square.svg",
					);
					renderInlineIcon(
						item.querySelector(".ws-delete span"),
						"assets/svgs-fontawesome/solid/trash.svg",
					);

					item.querySelector(".ws-open").onclick = () =>
						ws.urls.forEach((url) =>
	...
	widget.querySelector("#ws-add-btn").onclick = async () => {
		...
		tabs.forEach((tab) => {
			...
			row.innerHTML = `
				...
				}" class="ws-tab-icon" onerror="this.src='icons/icon16.png'">
	...
	widget.querySelector(".ws-save-btn").onclick = () => {
		const name =
			widget.querySelector(".ws-name-input").value.trim() ||
			"My Workspace";
		const selected = Array.from(
			widget.querySelectorAll(".ws-tabs-select input:checked"),
		).map((cb) => cb.value);
...
function createCalculatorWidget() {
	...
	widget.innerHTML = `
        ...
        <div class="widget-body">
            ...
            <div class="calc-grid">
                <button class="btn btn-small btn-text calc-btn" data-action="clear">C</button>
                <button class="btn btn-small btn-text calc-btn" data-action="delete">⌫</button>
                <button class="btn btn-small btn-text calc-btn" data-action="percent">%</button>
                <button class="btn btn-small btn-text calc-btn" data-action="divide">÷</button>
                <button class="btn btn-small btn-text calc-btn" data-num="7">7</button>
                <button class="btn btn-small btn-text calc-btn" data-num="8">8</button>
                <button class="btn btn-small btn-text calc-btn" data-num="9">9</button>
                <button class="btn btn-small btn-text calc-btn" data-action="multiply">×</button>
                <button class="btn btn-small btn-text calc-btn" data-num="4">4</button>
                <button class="btn btn-small btn-text calc-btn" data-num="5">5</button>
                <button class="btn btn-small btn-text calc-btn" data-num="6">6</button>
                <button class="btn btn-small btn-text calc-btn" data-action="subtract">−</button>
                <button class="btn btn-small btn-text calc-btn" data-num="1">1</button>
                <button class="btn btn-small btn-text calc-btn" data-num="2">2</button>
                <button class="btn btn-small btn-text calc-btn" data-num="3">3</button>
                <button class="btn btn-small btn-text calc-btn" data-action="add">+</button>
                <button class="btn btn-small btn-text calc-btn span-2" data-num="0">0</button>
                <button class="btn btn-small btn-text calc-btn" data-num=".">.</button>
                <button class="btn btn-small btn-text calc-btn" data-action="equals">=</button>
            </div>
        </div>
    `;
	renderInlineIcon(
		widget.querySelector(".widget-header .btn > span:first-child"),
		"assets/svgs-fontawesome/solid/calculator.svg",
	);
...
function createConverterWidget() {
	...
	widget.innerHTML = `
        ...
        <div class="widget-body">
            <div class="tools-chip-group conv-tabs">
                <button class="btn btn-small btn-text active" data-type="length">Length</button>
                <button class="btn btn-small btn-text" data-type="weight">Weight</button>
                <button class="btn btn-small btn-text" data-type="currency">Currency</button>
            </div>
            <div class="conv-form">
                <input type="number" id="conv-in" class="input" value="1">
                <select id="conv-from" class="select"></select>
                <div class="conv-arrow">→</div>
                <input type="number" id="conv-out" class="input" readonly>
                <select id="conv-to" class="select"></select>
            </div>
	...
	renderInlineIcon(
		widget.querySelector(".widget-header .btn > span:first-child"),
		"assets/svgs-fontawesome/solid/arrow-right-arrow-left.svg",
	);
	...
	widget.querySelectorAll(".conv-tabs button").forEach((btn) => {
		btn.onclick = async () => {
			widget
				.querySelectorAll(".conv-tabs button")
				.forEach((b) => b.classList.remove("active"));
			btn.classList.add("active");
			type = btn.dataset.type;

			if (type === "currency" && !rates) {
				widget.querySelector(".conv-rate").textContent =
					"Loading rates...";
				try {
...
function createQRWidget() {
	...
	widget.innerHTML = `
        ...
        <div class="widget-body">
            <input type="text" class="input qr-input" placeholder="Enter text or URL...">
            <div class="qr-output"></div>
            <button class="btn btn-small btn-text primary full-width qr-download-btn" disabled>Download QR</button>
        </div>
    `;
	renderInlineIcon(
		widget.querySelector(".widget-header .btn > span:first-child"),
		"assets/svgs-fontawesome/solid/qrcode.svg",
	);
	...
	const generateQR = () => {
		...
		if (!text) {
			output.innerHTML =
				'<div class="qr-placeholder">Enter text to generate QR</div>';
		...
		img.onerror = () => {
			output.innerHTML =
				'<div class="qr-error">Failed to generate QR</div>';
			downloadBtn.disabled = true;
		};
	};
...
function createTextToolsWidget() {
	...
	widget.innerHTML = `
        ...
        <div class="widget-body">
            <textarea class="textarea text-input" placeholder="Paste your text here..."></textarea>
            ...
            <div class="tools-chip-group text-actions">
                <button class="btn btn-small btn-text" data-action="upper" title="UPPERCASE">Aa→AA</button>
                <button class="btn btn-small btn-text" data-action="lower" title="lowercase">Aa→aa</button>
                <button class="btn btn-small btn-text" data-action="title" title="Title Case">Aa→Tt</button>
                <button class="btn btn-small btn-text" data-action="trim" title="Trim whitespace">Trim</button>
                <button class="btn btn-small btn-text" data-action="copy" title="Copy">Copy</button>
            </div>
        </div>
    `;
	renderInlineIcon(
		widget.querySelector(".widget-header .btn > span:first-child"),
		"assets/svgs-fontawesome/solid/font.svg",
	);
	...
	widget.querySelector(".text-actions").onclick = (e) => {
		...
		switch (action) {
			...
			case "lower":
				...
				break;
			case "title":
				textarea.value = textarea.value.replace(/\b\w/g, (c) =>
					c.toUpperCase(),
				);
				break;
			case "trim":
...
function createPasswordWidget() {
	...
	widget.innerHTML = `
        <div class="widget-header">
            ...
            <button class="btn btn-icon btn-small widget-action" title="Generate"><span></span></button>
        </div>
        <div class="widget-body">
            <div class="password-output">
                <input type="text" class="input password-display" readonly>
                <button class="btn btn-icon btn-small password-copy" title="Copy"><span></span></button>
            </div>
            <div class="password-strength"></div>
            <div class="password-options">
                <div class="password-length">
                    <label>Length: <span class="length-value">16</span></label>
                    <input type="range" class="input-range length-slider" min="8" max="32" value="16">
                </div>
	...
	renderInlineIcon(
		widget.querySelector(".widget-header .btn > span:first-child"),
		"assets/svgs-fontawesome/solid/key.svg",
	);
	renderInlineIcon(
		widget.querySelector(".widget-action span"),
		"assets/svgs-fontawesome/solid/rotate.svg",
	);
	renderInlineIcon(
		widget.querySelector(".password-copy span"),
		"assets/svgs-fontawesome/solid/copy.svg",
	);
	...
	const generate = () => {
		...
		widget
			.querySelectorAll(".password-toggles input:checked")
			.forEach((cb) => {
...
function createJsonWidget() {
	...
	widget.innerHTML = `
        ...
        <div class="widget-body">
            <textarea class="textarea json-input" placeholder="Paste JSON here..."></textarea>
            <div class="tools-chip-group json-actions">
                <button class="btn btn-small btn-text" data-action="format">Format</button>
                <button class="btn btn-small btn-text" data-action="minify">Minify</button>
                <button class="btn btn-small btn-text" data-action="copy">Copy</button>
            </div>
	...
	renderInlineIcon(
		widget.querySelector(".widget-header .btn > span:first-child"),
		"assets/svgs-fontawesome/solid/code.svg",
	);
...
function createBase64Widget() {
	...
	widget.innerHTML = `
        ...
        <div class="widget-body">
            <textarea class="textarea base64-input" placeholder="Enter text..."></textarea>
            <div class="tools-chip-group base64-actions">
                <button class="btn btn-small btn-text" data-action="encode">Encode</button>
                <button class="btn btn-small btn-text" data-action="decode">Decode</button>
                <button class="btn btn-small btn-text" data-action="copy">Copy</button>
            </div>
        </div>
    `;
	renderInlineIcon(
		widget.querySelector(".widget-header .btn > span:first-child"),
		"assets/svgs-fontawesome/solid/lock.svg",
	);
	...
	widget.innerHTML = `
        ...
        <div class="widget-body">
            <div class="lorem-options">
                <select class="select lorem-type">
                    <option value="paragraphs">Paragraphs</option>
                    <option value="sentences">Sentences</option>
                    <option value="words">Words</option>
                </select>
                <input type="number" class="input lorem-count" value="2" min="1" max="10">
            </div>
            <div class="lorem-output"></div>
            <button class="btn btn-small btn-text primary full-width lorem-copy">Copy</button>
        </div>
    `;
	renderInlineIcon(
		widget.querySelector(".widget-header .btn > span:first-child"),
		"assets/svgs-fontawesome/solid/paragraph.svg",
	);
	renderInlineIcon(
		widget.querySelector(".widget-action span"),
		"assets/svgs-fontawesome/solid/rotate.svg",
...
function createRegexWidget() {
	...
	widget.innerHTML = `
        ...
        <div class="widget-body">
            <input type="text" class="input regex-pattern" placeholder="Pattern (e.g. \\d+)">
            <input type="text" class="input regex-flags" placeholder="Flags (e.g. gi)" value="g">
            <textarea class="textarea regex-text" placeholder="Test text..."></textarea>
            <div class="regex-result"><span style="color:var(--text-off); font-style:italic;">Result will appear here...</span></div>
        </div>
    `;
	renderInlineIcon(
		widget.querySelector(".widget-header .btn > span:first-child"),
		"assets/svgs-fontawesome/solid/asterisk.svg",
	);
	...
	const test = () => {
		...
		try {
			...
			if (matches) {
				result.innerHTML = `<span style="color:#00cc44">${
					matches.length
				} match${matches.length > 1 ? "es" : ""}</span>: ${matches.join(
					", ",
				)}`;
			} else {
				result.innerHTML =
					'<span style="color:#ff8844">No matches</span>';
			}
		} catch (e) {
...
function createHashWidget() {
	...
	widget.innerHTML = `
        ...
        <div class="widget-body">
            <input type="text" class="input hash-input" placeholder="Enter text to hash...">
            <div class="hash-results"></div>
        </div>
    `;
	renderInlineIcon(
		widget.querySelector(".widget-header .btn > span:first-child"),
		"assets/svgs-fontawesome/solid/fingerprint.svg",
	);
	...
	algos.forEach((algo) => {
		...
		row.innerHTML = `
            ...
            <input class="input hash-value" readonly>
            <button class="btn btn-icon btn-small hash-copy" title="Copy"><span></span></button>
        `;
		renderInlineIcon(
			row.querySelector(".hash-copy span"),
			"assets/svgs-fontawesome/solid/copy.svg",
		);
	...
	const hashText = async (text) => {
		...
		if (!text) {
			rows.forEach(
				(row) => (row.querySelector(".hash-value").value = ""),
			);
			return;
		}
...
function createRssWidget() {
	...
	widget.innerHTML = `
        <div class="widget-header">
            ...
            <button class="btn btn-icon btn-small widget-action" title="Refresh"><span></span></button>
        </div>
        <div class="widget-body">
            <div class="rss-url-row">
                <input type="text" class="input rss-url" placeholder="Enter RSS feed URL...">
                <button class="btn btn-small btn-text primary rss-load">Load</button>
            </div>
	...
	renderInlineIcon(
		widget.querySelector(".widget-header .btn > span:first-child"),
		"assets/svgs-fontawesome/solid/rss.svg",
	);
	renderInlineIcon(
		widget.querySelector(".widget-action span"),
		"assets/svgs-fontawesome/solid/rotate.svg",
		...
		try {
			...
			if (items.length === 0) {
				itemsContainer.innerHTML =
					'<div class="rss-empty">No items found</div>';
				return;
			}
			...
			itemsContainer.innerHTML =
				'<div class="rss-error">Failed to load feed</div>';
		}
	};
