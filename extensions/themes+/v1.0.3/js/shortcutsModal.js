...
export async function openShortcutModal(shortcut = null, onSave) {
    ...
    const createField = (label, value, placeholder) => {
        ...
        input.className = 'setting-select'; // Reuse input style
        input.style.width = '100%';
        input.style.backgroundImage = 'none'; // reset select arrow
    ...
    const urlField = createField('URL', shortcut?.url, 'e.g. https://example.com');
    modal.appendChild(urlField.wrap);
    
    const iconField = createField('Icon URL (Optional)', shortcut?.icon, 'https://...');
    ...
    saveBtn.className = 'confirm-btn confirm';
