...
const app = (typeof chrome !== 'undefined' ? chrome : (typeof browser !== 'undefined' ? browser : null));
...
export function resolveAssetUrl(pathOrUrl) {
    ...
    try {
        ...
        // Treat as relative path on the known host
...
async function fetchAndStoreBlob(url, key, meta = {}, maxAttempts = 3) {
    ...
    try {
        const entry = Object.assign({ key, blob, status, errorMessage, updatedAt: Date.now() }, meta || {});
        await putMediaEntry(entry);
    } catch (dbErr) {
        // ** Use concise identifier in log **
        console.warn(`Failed to persist media entry ${logIdentifier}`, dbErr);
        return { ok: false, error: dbErr && dbErr.message ? dbErr.message : String(dbErr) };
    }
    ...
    if (status === 'ok') {
        ...
        // ** Use concise identifier in log **
        console.warn(`Resource download failed for ${logIdentifier}: ${errorMessage}`);
...
export async function downloadAndStoreResource({ url, assetPath, kind = 'thumb', manifestEntry = null, maxAttempts = 3 }) {
    ...
    const meta = {
        ...
        version: manifestEntry && manifestEntry.version ? manifestEntry.version : null,
